...
def summarize_document(file_obj, history, max_tokens, temperature, top_p, system_instruction):
    history = history or []
    try:
        ...
        resp = gemini.models.generate_content(
            ...
            contents=[
                ...
                types.Part(text="Vui lòng tóm tắt ngắn gọn bằng tiếng Việt.")
            ]
        )
...
def describe_image(image, history, max_tokens, temperature, top_p, system_instruction):
    history = history or []
    try:
        buf = BytesIO()
        ...
        resp = gemini.models.generate_content(
            ...
            contents=[
                ...
                types.Part(text="Mô tả chi tiết hình ảnh bằng tiếng Việt.")
            ]
        )
        history.append(("🖼️ Mô tả:", resp.text))
    except Exception as e:
        traceback.print_exc()
        history.append(("❌ Lỗi mô tả ảnh:", str(e)))
    return history
